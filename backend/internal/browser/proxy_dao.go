...
func (d *SQLiteProxyDAO) Upsert(proxy Proxy) error {
	...
	_, err := d.db.Exec(`
		INSERT INTO browser_proxies (
		  proxy_id, proxy_name, proxy_config, dns_servers, country, region, city, timezone, locale, group_name,
		...
		proxy.ProxyId, proxy.ProxyName, proxy.ProxyConfig, proxy.DnsServers,
		proxy.Country, proxy.Region, proxy.City, proxy.Timezone, proxy.Locale, proxy.GroupName,
...
func scanProxies(rows *sql.Rows) ([]Proxy, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&p.ProxyId, &p.ProxyName, &p.ProxyConfig, &p.DnsServers,
			&p.Country, &p.Region, &p.City, &p.Timezone, &p.Locale, &p.GroupName,
